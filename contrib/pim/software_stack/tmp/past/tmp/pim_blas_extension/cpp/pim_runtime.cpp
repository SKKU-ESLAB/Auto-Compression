...
void GetMicrokernelCode(PIM_OP op, PIM_OP_ATTRS op_attrs) {
	std::cout << "GetMicrokernelCode!\n";
}

// PIM Memory Manager
//bool AllocMem(PIMMem pim_mem) {
void AllocMem(PIMMem pim_mem) {
	std::cout << "AllocMem!\n";
}
...
void ReadMem(PIMMem pim_mem, uint8_t *data, size_t len) {
	std::cout << "ReadMem!\n";
}

//size_t WriteMem(PIMMem pim_mem, uint8_t *data, size_t len) {
void WriteMem(PIMMem pim_mem, uint8_t *data, size_t len) {
	std::cout << "WriteMem!\n";
}

// PIM Kernel Executor
//bool ExecuteKernel(PIMKernel pim_kernel) {
void ExecuteKernel(PIMKernel pim_kernel) {
