...
/* ----------------------------------------------------------------------
 * Project:      TinyEngine
 * Description:  for sparse in-place 1x1 depth-wise convolution (HWC->CHW->HWC)
 * Target ISA:  ARMv7E-M
 * Author: wmchen@mit.edu
 * -------------------------------------------------------------------- */
#include "arm_nnsupportfunctions.h" //TODO: remove this in the future for self-contained
#include "tinyengine_function.h"
void depthwise_kernel1x1_stride1_inplace_kernel_CHW_fpreq_bitmask(
...
tinyengine_status depthwise_kernel1x1_stride1_inplace_CHW_fpreq_bitmask(q7_t *input, const uint16_t input_x, const uint16_t input_y,
            ...
            const uint16_t output_ch, q15_t *runtime_buf, q7_t pad_value)
{
    ...
    for (c = 0; c < input_ch; c++){
        cols_8b = (q7_t*)(cols_8b_start + 0 * (input_x) + 0); //skip 0 rows
        src = input;
        for(i = 0; i < input_y; i++){
            cols_8b += 0;//skip front
            for(j = 0; j < input_x; j++){
        ...
        depthwise_kernel1x1_stride1_inplace_kernel_CHW_fpreq_bitmask(output_y, output_x, bias++, biasR++, ksrc, scales++, inplace_out, output_mask, c % 8, output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x, input_ch);
        ksrc += 1;
        input++;
