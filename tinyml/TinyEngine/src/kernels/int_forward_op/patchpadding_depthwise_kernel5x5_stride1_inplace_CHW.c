...
void patch_depthwise_kernel5x5_stride1_inplace_kernel_CHW(
...
tinyengine_status patchpadding_depthwise_kernel5x5_stride1_inplace_CHW(q7_t *input, const uint16_t input_x, const uint16_t input_y,
				...
				const uint16_t pad_t, const uint16_t pad_b, const uint16_t pad_l, const uint16_t pad_r)
{
	...
	for (c = 0; c < input_ch; c++){        
		...
		patch_depthwise_kernel5x5_stride1_inplace_kernel_CHW(output_y, output_x, bias++, biasR++, ksrc, output_mult++, output_shift++, inplace_out, output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x, input_ch);
		inplace_out++;
		input++;
		ksrc += 25;
    }
