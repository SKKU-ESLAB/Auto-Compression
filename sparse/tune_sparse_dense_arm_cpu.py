...
def apply_func(search_policy, state, stage_id):
    ...
    if ARGS.output_layout == 'hwc':
        ...
        i0, i1, i2, i3 = s0.split(sparse_dense_block, i, [None, None, None])
        m0, m1, m2 = s0.follow_split(consumer, m, len(s0.transform_steps) - 1, 2)

        j0, j1 = s0.split(sparse_dense_block, nb_j, [None])
        n0, n1 = s0.follow_split(consumer, nb_n, len(s0.transform_steps) - 1, 1)

        c0, c1, c2 = s0.split(sparse_dense_block, c, [None, None])

        s0.reorder(sparse_dense_block, [i0, j0, i1, j1, c0, row_offset, c1, i2, c2, i3, j])
        s0.reorder(consumer, [m0, n0, m1, n1, m2, n])
        s0.compute_at(sparse_dense_block, consumer, n1)
        ...
        s0.reorder(sparse_dense_block, [j0, i0, j1, i1, c0, row_offset, c1, i2, c2, j, i3])
        s0.reorder(consumer, [n0, m0, n1, m1, n, m2])
        s0.compute_at(sparse_dense_block, consumer, m1)
...
log_file = "spmm_arm_cpu_hw%d_ci%d_co%d_d%s_bsi%d_bso%d_%s_%s_%s.json" % (ARGS.hw, ARGS.ci, ARGS.co, str(ARGS.density), ARGS.bso, ARGS.bsi, ARGS.data_layout, ARGS.weight_layout, ARGS.output_layout)
