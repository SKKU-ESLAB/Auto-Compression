...
def _sparse_dense_sp_rhs_bsrmm_v2(data, weight_data, weight_indices, weight_indptr):
    ...
    return te.compute(
        (num_blocks * bs_r, m),
        lambda n, m: bsrmm_block[idxd(n, bs_r), idxm(n, bs_r), m],
        tag="sparse_dense_sp_rhs_bsrmm",
    )

def _sparse_dense_sp_rhs_bsrmm_v3(data, weight_data, weight_indices, weight_indptr):
    (k, m) = get_const_tuple(data.shape)
    (_, bs_r, bs_c) = get_const_tuple(weight_data.shape)
    (num_blocks_plus_1,) = get_const_tuple(weight_indptr.shape)
    num_blocks = num_blocks_plus_1 - 1
...
def _sparse_dense_sp_rhs_bsrmm_v4(data, weight_data, weight_indices, weight_indptr):
...
@auto_scheduler.register_workload
def sparse_dense_v2(M, N, K, w_data_shape, w_indices_shape, w_indptr_shape, dtype):
    X = te.placeholder(shape=(K, M), dtype=dtype)
    W_data = te.placeholder(shape=w_data_shape, dtype=dtype)
    W_indices = te.placeholder(shape=w_indices_shape, dtype="int32")
    W_indptr = te.placeholder(shape=w_indptr_shape, dtype="int32")

    out = _sparse_dense_sp_rhs_bsrmm_v2(X, W_data, W_indices, W_indptr)
...
# (k, m) -> (m, n)
@auto_scheduler.register_workload
def sparse_dense_v3(M, N, K, w_data_shape, w_indices_shape, w_indptr_shape, dtype):
    X = te.placeholder(shape=(K, M), dtype=dtype)
    W_data = te.placeholder(shape=w_data_shape, dtype=dtype)
    W_indices = te.placeholder(shape=w_indices_shape, dtype="int32")
    W_indptr = te.placeholder(shape=w_indptr_shape, dtype="int32")

    out = _sparse_dense_sp_rhs_bsrmm_v3(X, W_data, W_indices, W_indptr)
...
# (m, k) -> (n, m)
@auto_scheduler.register_workload
def sparse_dense_v4(M, N, K, w_data_shape, w_indices_shape, w_indptr_shape, dtype):
    X = te.placeholder(shape=(M, K), dtype=dtype)
    W_data = te.placeholder(shape=w_data_shape, dtype=dtype)
    W_indices = te.placeholder(shape=w_indices_shape, dtype="int32")
    W_indptr = te.placeholder(shape=w_indptr_shape, dtype="int32")

    out = _sparse_dense_sp_rhs_bsrmm_v4(X, W_data, W_indices, W_indptr)
...
def apply_func(search_policy, state, stage_id):
    ...
    s0.reorder(consumer, [m0, n0, m1, n1, m2, n2, n, m3])
...
s, bufs = dag.apply_steps_from_state(inp.state, layout_rewrite=auto_scheduler.LayoutRewriteOption.INSERT_TRANSFORM_STAGE)
