...
parser.add_argument('--ks', default=1, type=int)
parser.add_argument('--str', default=1, type=int)
...
def try_get_sparse_conv2d_input(args):
    ...
    def _traverse(t):
        ...
        if isinstance(t.op, te.ComputeOp):
            ...
            if t.op.tag[:-4] == "sparse_conv2d":
                assert len(t.op.input_tensors) == 1
                block_tensor = t.op.input_tensors[0]
                if block_tensor.op.tag == "sparse_conv2d_block_hwc":
                    m, n = t.shape
                elif block_tensor.op.tag == "sparse_conv2d_block_chw":
                    n, m = t.shape
                _process_inputs(block_tensor.op.input_tensors, m, n, "sparse_conv2d_bsr")
...
def _sparse_conv2d_sp_rhs_bsrmm(data, weight_data, weight_indices, weight_indptr,
...
def sparse_conv2d(data_shape, w_data_shape, w_indices_shape, w_indptr_shape, dtype, data_layout, weight_layout, output_layout):
    ...
    out = _sparse_conv2d(X, W_data, W_indices, W_indptr, data_layout, weight_layout, output_layout)
...
prefix = "sparse_conv2d_v2_bsr_%d_%d_%d_%d_%.2f_" % (W_np.shape[0], W_np.shape[1], W_sp_np_data.shape[1], W_sp_np_data.shape[2], density)
...
def meet_condition_func(search_policy, state, stage_id):
    ...
    if state.stages[stage_id].op.tag in [
        "sparse_conv2d_hwc",
        "sparse_conv2d_chw",
        "sparse_conv2d_block_hwc",
        "sparse_conv2d_block_chw"
    ]:
...
def apply_func(search_policy, state, stage_id):
    ...
    if s0.stages[stage_id].op.tag[:-4] == "sparse_conv2d_block":
        return [s0.state_object, stage_id - 1]

    sparse_conv2d = s0.stages[stage_id].op
    sparse_conv2d_block = s0.stages[stage_id - 1].op
    assert sparse_conv2d.tag[:-4] == "sparse_conv2d"
    assert sparse_conv2d_block.tag[:-4] == "sparse_conv2d_block"
    ...
    if ARGS.output_layout == 'hwc':
        ...
        i0, i1, i2, i3, i4 = s0.split(sparse_conv2d_block, i, [None, None, None, None])
        m0, m1, m2, m3 = s0.follow_split(consumer, m, len(s0.transform_steps) - 1, 3)

        j0, j1, j2 = s0.split(sparse_conv2d_block, nb_j, [None, None])
        #n0, n1, n2 = s0.follow_split(consumer, nb_n, len(s0.transform_steps) - 1, 2)
        n0, n1, n2, n3 = s0.follow_split(consumer, n, len(s0.transform_steps) - 1, 3)

        c0, c1, c2 = s0.split(sparse_conv2d_block, c, [None, None])

        s0.reorder(sparse_conv2d_block, [i0, j0, i1, j1, i2, j2, c0, row_offset, c1, i3, c2, i4, j])
        s0.reorder(consumer, [m0, n0, m1, n1, m2, n2, m3, n3])
        s0.compute_at(sparse_conv2d_block, consumer, n2)
    elif ARGS.output_layout == 'chw':
        nb_j, j, i, row_offset, c = s0[sparse_conv2d_block].iters
        ...
        s0.reorder(sparse_conv2d_block, [j0, i0, j1, i1, j2, i2, c0, row_offset, c1, i3, c2, j, i4])
        s0.reorder(consumer, [n0, m0, n1, m1, n2, m2, n3, m3])
        s0.compute_at(sparse_conv2d_block, consumer, m2)
...
log_file = "spconv2d_mali_hw%d_ci%d_co%d_d%s_bsi%d_bso%d_%s_%s_%s.json" % (ARGS.hw, ARGS.ci, ARGS.co, str(ARGS.density), ARGS.bso, ARGS.bsi, ARGS.data_layout, ARGS.weight_layout, ARGS.output_layout)
