...
def replace_module_by_names(model, replaced_modules, pruning_layers):
    def helper(childs: nn.Module):
        for name, child in childs.named_children():
            if type(child) is nn.Conv2d:
                for full_name, module in model.named_modules():
                    if (child is module) and (child.kernel_size == (1, 1)):
                        if full_name in pruning_layers:
...
def change_threshold(m):
    if hasattr(m, "threshold"):
        m.threshold = (m.threshold - 0.05) if (m.threshold > 0.25) else 0.25