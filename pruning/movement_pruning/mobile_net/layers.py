...
class DepthConvLayer(My2DLayer):
    ...
    def module_str(self):
        if isinstance(self.kernel_size, int):
            ...
            kernel_size = self.kernel_size
        if self.dilation > 1:
            conv_str = '%dx%d_DilatedDepthConv' % (
                kernel_size[0], kernel_size[1])
        else:
...
class LinearLayer(MyModule):
    ...
    def __init__(self, in_features, out_features, bias=True,
        ...
        if self.dropout_rate > 0:
            ...
            modules['dropout'] = None
        # linear
        modules['weight'] = {'linear': nn.Linear(
...
class MvPrunConv2d(nn.Conv2d):
    def __init__(self, m: nn.Conv2d,
        ...
        if self.pruning_method in ["topK", "threshold", "sigmoied_threshold", "l0"]:
            ...
            mask_size = (size[0] // self.mask_block_rows, size[1] // self.mask_block_cols, size[2], size[3])
    ...
    @staticmethod
    def check_name(name):
        return name.endswith(".mask_scores")
    ...
    def mask_(weight,
        ...
        if pruning_method != "magnitude":
            mask = MvPrunConv2d.expand_mask_(mask,
    ...
    def masked_weights_from_state_dict(state_dict,
        ...
        parameters = {}
        for name in ["weight", "mask_scores"]:
            parameters[name] = state_dict.get(name_for_mask(weight_name, name))
            
        ret = MvPrunConv2d.masked_weights(pruning_method=pruning_method,
                                          threshold=threshold,
                                          training=False,
                                          mask_block_rows=mask_block_rows,
                                          mask_block_cols=mask_block_cols,
    ...
    def forward(self, inputs):
        ...
        return self._conv_forward(inputs, weight_thresholded, self.bias)