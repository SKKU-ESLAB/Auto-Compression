...
# Pruning-related aruguments
parser.add_argument('--prune-amount', default=0.5, type=float,
                    help='Pruning amount')
...
def _compute_block_norm(t, n, block_size=(4, 1)):
    block_t = t.reshape(t.shape[0] // block_size[0], block_size[0], t.shape[1] // block_size[1], block_size[1], *t.shape[2:])
    norm = torch.norm(block_t, p=n, dim=(1, 3))
    return norm

class LnBlockUnstructured(prune.BasePruningMethod):
    PRUNING_TYPE = 'unstructured'

    def __init__(self, amount, n, block_size=(4, 1)):
    ...
    @classmethod
    def apply(cls, module, name, amount, n, block_size, importance_scores=None):
        return super(LnBlockUnstructured, cls).apply(
            module,
            name,
            amount=amount,
            n=n,
...
def ln_block_unstructured(module, name, amount, n, block_size, importance_scores=None):
    LnBlockUnstructured.apply(
        module, name, amount, n, block_size, importance_scores=importance_scores
    )
    return module
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # optionally restart from a checkpoint
    if args.restart:
        if os.path.isfile(args.restart):
            print("=> loading checkpoint '{}'".format(args.restart))
            if args.gpu is None:
                checkpoint = torch.load(args.restart)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                checkpoint = torch.load(args.restart, map_location=loc)
            args.start_epoch = 0
...
def train(train_loader, model, teacher_model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            soft_label = nn.functional.softmax(soft_logits, dim=1)
