...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet20','resnet18_2']
...
class GroupBottleneck2(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.relu(out)
        return out
class CIFAR_GroupedMaskedResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, group_shape=(1, 4), grouped_rule='l1'):
        super(CIFAR_GroupedMaskedResNet, self).__init__()
        self.in_planes = 16       
    ...
    def forward(self, x):
        ...
        out = self.linear(out)
        return out
class GroupedMaskedResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, group_shape=(1, 4), grouped_rule='l1'):
        super(GroupedMaskedResNet, self).__init__()
        self.in_planes = 64

        #if first_last:
        #    self.conv1 = AlignedGroupedMaskedConv2d(3, 64, kernel_size=(3, 3), stride=1, padding=1, bias=False,
        #              group_shape=group_shape, grouped_rule=grouped_rule)
        #else:
        # 생각해보니; 첫번째는 (64,3,3,3)의 쉐이프라서 (64, 1, -1, 4)의 형태로 변형이 안되네;;
    ...
    def forward(self, x):
        #out = self.maxpool1(F.relu(self.bn1(self.conv1(x))))
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
        #out = F.avg_pool2d(out, 4) # 커널 사이즈 4로 하는건데, 사이즈 안맞을 수 있음
...
class ResNet2(nn.Module):
    ...
    def forward(self, x):
        return self._forward_impl(x)
###############################################
def resnet20(num_classes=10, group_shape=(1, 4), grouped_rule='l1', prune_type = 'group'):
    if prune_type =='group':
        return CIFAR_GroupedMaskedResNet(GroupBasicBlock2, [3, 3, 3], num_classes=num_classes, group_shape=group_shape, grouped_rule=grouped_rule)
    elif prune_type =='filter':
        return CIFAR_GroupedMaskedResNet(FilterBasicBlock, [3, 3, 3], num_classes=num_classes, group_shape=group_shape, grouped_rule=grouped_rule)
def resnet18_2(num_classes=10, group_shape=(1, 4), grouped_rule='l1', prune_type = 'group'):
    if prune_type =='group':
        return GroupedMaskedResNet(GroupBasicBlock2, [2, 2, 2, 2], num_classes=num_classes, group_shape=group_shape, grouped_rule=grouped_rule)
    elif prune_type =='filter':
        return GroupedMaskedResNet(FilterBasicBlock2, [2, 2, 2, 2], num_classes=num_classes, group_shape=group_shape, grouped_rule=grouped_rule)
