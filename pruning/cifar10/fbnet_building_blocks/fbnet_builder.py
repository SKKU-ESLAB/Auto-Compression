...
PRIMITIVES = {
    ...
    "A1_W1": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 1, w, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A2_W1": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 2, w, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A3_W1": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 3, w, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A4_W1": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 4, w, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A1_W2": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 1, w+1, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A2_W2": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
            C_in, C_out, 2, w+1, 3, stride, padding, maxpool, 0, "relu", "bn", index, layer_num, False, **kwargs
    ),
    "A3_W2": lambda C_in, C_out, a, w, stride, padding, maxpool, index, layer_num, **kwargs: QConvBNRelu(
    ...
    def __init__(
        ...
        if in_planes==3:
            ...
            if type_prune=='channel':
                ...
                tmp_pruned = tmp_pruned.pow(2.0).mean(2, keepdim=True).pow(0.5).expand(tmp_pruned.shape)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, type_prune, sparsity, layer_num, option='A'):
        ...
        if type_prune=='channel':
            ...
            tmp_pruned = tmp_pruned.contiguous().view(original_size)
            self.conv2 = prune.custom_from_mask(self.conv2, name='weight', mask=tmp_pruned)
