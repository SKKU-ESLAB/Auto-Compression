...
RotaryEmbedding::RotaryEmbedding(SimulationConfig config, Model* model,
    ...
    std::cout << _input_shape.at(0) <<"?"<< std::endl; // 32000
    std::cout << _input_shape.at(1) << std::endl; // 4096

    _skip_shape = get_input(1)->get_dims();
    //std::cout << _skip_shape.at(0) << std::endl;  // 1
    std::cout << _skip_shape.at(1) << std::endl;  // 64
    _weight_shape = get_input(2)->get_dims();
    //std::cout << _weight_shape.at(0) << std::endl;  // 4096
    //std::cout << _weight_shape.at(1) << std::endl;  // 64
    _dense_bias_shape = get_input(3)->get_dims();
    //std::cout << _dense_bias_shape.at(0) << std::endl; //4096
    std::cout << _dense_bias_shape.at(1) << std::endl; //64
    assert(_input_shape.size()==3);
    _batch_size = 1;//_input_shape.at(0);
    _seq = 64;//_input_shape.at(0);
    _dk = 128;//_input_shape.at(1);
...
void RotaryEmbedding::initialize_tiles(MappingTable& mapping_table) {
    if(_outputs.size() == 0) {
        _input_shape = get_input(0)->get_dims();
        _output_shape = _input_shape;
...
void RotaryEmbedding::initialize_instructions(Tile* tile, Mapping mapping, uint32_t token_offset, uint32_t tokens) {
...
void RotaryEmbedding::calculate_loops() {
    uint32_t size_per_token = 64 * 2 * _config.precision;
    ...
    spdlog::info("[RotaryEmbedding] tokens_per_tile: {}", _tokens_per_tile);
}