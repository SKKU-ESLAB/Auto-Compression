...
Attention::Attention(SimulationConfig config, Model* model,
    ...
    _dmodel = 4096;//_weight_shape.at(0);
    _nkvh = _nh;
    _dk = _dmodel / _nh;
    _q_len = 64;
    _seq = 64;
    ...
    _liner_output_shape = std::vector<uint32_t>{_batch_size, _q_len, 4096};
    _projection_output_shape = std::vector<uint32_t>{_batch_size, _q_len, 1365};
...
Attention::Attention(SimulationConfig config, Model* model, 
    ...
    _nkvh = _nh;//std::stoi(get_attribute("num_kv_heads"));
    _dmodel = 4096;//std::stoi(get_attribute("hidden_size"));
    _dk = _dmodel / _nh;
}
...
void Attention::initialize_tiles(MappingTable& mapping_table) {
    if(_outputs.empty()) {
        ...
        _model->add_tensor(std::move(output_tensor));
        _input_shape = get_input(0)->get_dims();
        _seq = 64;//get_input(1)->get_dims()[0]; //key first dim
        _weight_shape = get_input(1)->get_dims();
        _liner_output_shape = std::vector<uint32_t>{_q_len, 4096};
        _query_shape = std::vector<uint32_t>{_nh, _q_len, _dk};
...
void Attention::initialize_onnx_tiles(MappingTable& mapping_table) {
    ...
    /* Fused Attention body */
    for (int req_idx = 0; req_idx < 1; req_idx++) {
        int heads_per_tile = _heads_per_tile[req_idx];
        for (int head_off=0; head_off<4; head_off+=heads_per_tile) {
...
void Attention::initialize_instructions(Tile* tile, int head_idx, int num_heads) {
    ...
    addr_type sram_k_ofs = sram_key_base + kv_head_idx * (_dk * 2 * seq_len) * _config.precision;
    ...
    for (int h_ofs = 0; h_ofs < num_heads; h_ofs++) {
        ...
        addr_type sram_logits_offset = sram_l_ofs + h_ofs * (q_len * seq_len) * _config.precision;
        ...
        // GEMM (q*k -> l)
        /*tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
            .opcode = Opcode::MASKEDGEMM_PRELOAD,
        ...
        for(int sitr = 0; sitr < seq_len; sitr+=_config.core_config[target_core].core_height) {
            for(int kitr = 0; kitr < q_len; kitr+=_config.core_config[target_core].core_height) {
                addr_type sram_q_ofs = sram_query_base + h_ofs * (kitr * _dk) * _config.precision;
                addr_type sram_l_ofs = sram_logit_base + h_ofs * (kitr * sitr) * _config.precision;
                addr_type sram_k_ofs = sram_key_base + kv_head_idx * (_dk * sitr) * _config.precision;
                if(kitr == 0){
                    tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
                        .opcode = Opcode::GEMM_PRELOAD,
                        .dest_addr = sram_logit_base + h_ofs * (kitr * sitr) * _config.precision,
                        .size = _config.core_config[target_core].core_height * _config.core_config[target_core].core_height * _config.precision / _config.dram_req_size,
                        .compute_size = q_len,//ceil_div(q_len, _config.core_config[target_core].core_height) * seq_len,
                        .src_addrs = std::vector<addr_type>{sram_q_ofs, sram_k_ofs},

                        .tile_m = _config.core_config[target_core].core_height,
                        .tile_k = _dk,
                        .tile_n = _config.core_config[target_core].core_height,
                    }));  
                }
                else{
                    tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
        ...
        for(int kitr = 0; kitr < seq_len; kitr+=_config.core_config[target_core].core_height) {
            for(int sitr = 0; sitr < q_len; sitr+=_config.core_config[target_core].core_height) {
                addr_type sram_l_ofs = sram_logit_base + h_ofs * (kitr * sitr) * _config.precision;
                addr_type sram_v_ofs = sram_value_base + h_ofs * (kitr * sitr) * _config.precision;
                if(sitr == 0){
                    tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
                        .opcode = Opcode::GEMM_PRELOAD,
                        .dest_addr = sram_l_ofs,
                        .size = _config.core_config[target_core].core_height * _dk * _config.precision / _config.dram_req_size,
                        .compute_size = q_len,//ceil_div(q_len, _config.core_config[target_core].core_height) * seq_len,
                        .src_addrs = std::vector<addr_type>{sram_l_ofs, sram_v_ofs},
                        .tile_m = _dk,
                        .tile_k = _config.core_config[target_core].core_height,
                        .tile_n = _config.core_config[target_core].core_height,
                        .src_from_accum = true,
                    }));
                }
