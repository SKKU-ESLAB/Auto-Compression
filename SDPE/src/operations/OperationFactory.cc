...
std::unique_ptr<Operation> OperationFactory::create_operation(
  ...
  if (node_proto.op_type() == "Conv" || node_proto.op_type() == "FusedConv") {
    ...
    return std::make_unique<Flatten>(_config, model, node_proto, target_core);
             ...
             node_proto.op_type() == "MultiHeadAttention") {
  ...
  } else if (node_proto.op_type() == "SkipSimplifiedLayerNormalization"||
             node_proto.op_type() == "SkipLayerNormalization") {
  ...
  } else if (node_proto.op_type() == "RotaryEmbedding") {
    return std::make_unique<RotaryEmbedding>(_config, model, node_proto, target_core);
  }
