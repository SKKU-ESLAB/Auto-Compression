...
void GemmWS::initialize_instructions(Tile* tile, Mapping mapping) {
  ...
  for (int Ms = 0; Ms < mapping.tile_in_loop.M; Ms += loop_size) {
    ...
    for (int Cs = 0; Cs < mapping.tile_in_loop.C; Cs+= loop_size) {
      ...
      int c_in_loop = C_offset + loop_size > mapping.total_loop.C
    ...
    for (int Cs = 0; Cs < mapping.tile_in_loop.C; Cs+= loop_size) {
      ...
      for (int Ns = 0; Ns < mapping.tile_in_loop.N; Ns += loop_size) {
        ...
        /*for(int c_iter = 0; c_iter < c_in_loop; c_iter+=_config.core_config[target_core].core_height) {
          ...
          //std::cout <<n_loop<< mapping.tile_in_loop.M << " " << mapping.tile_in_loop.C << " " << mapping.tile_in_loop.N << c_in_loop<<std::endl;
          //std::cout << "GEMM_PRELOAD" << (uint32_t)n_loop << std::endl;
          tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
          ...
          //if (c_iter%mapping.tile_in_loop.N == 0) {

          if (Ns == 0) {  // c= 128 전용
            //std::cout << "GEMV_PRELOAD" << (uint32_t)n_loop << std::endl;
            tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
            ...
            //std::cout << "GEMV" << (uint32_t)n_loop << std::endl;
            tile->instructions.push_back(std::make_unique<Instruction>(Instruction{
                .opcode = Opcode::GEMM,
                .dest_addr = out_sp_addr,
                .size = (uint32_t)n_loop,
