...
void Model::initialize_model(std::vector<std::unique_ptr<Tensor>>& weight_table) {
  ...
  for (auto iter: input) {
    ...
    for (int dim_idx=0; dim_idx<input_shape.dim_size(); dim_idx++) {
      ...
      std::cout << "input name: " << input_name << " val: "  << dim_value << " param : "<< dim_param << std::endl;
  ...
  for(auto node_proto : model_proto.graph().node()) {
    ...
    if(node != nullptr) {
      ...
      if (node_proto.op_type() == "SkipSimplifiedLayerNormalization" && _model_config.contains("nr_atten"))
  ...
  for(auto& [key, val] : _operation_map) {
    ...
    if (val->get_optype() == "MultiHeadAttention") {
      ...
      //_operation_map[projection_id]->_name = "QKV Generation";
    }
    else {
...
bool Model::check_regressive() {
  if (_axis_map.find("past_sequence_length + sequence_length") == _axis_map.end()){
    spdlog::info("No past_sequence_length + sequence_length!");
    return false;
  }
  if ((_axis_map["past_sequence_length + sequence_length"]+1) == _model_config["output_seq_len"]) {
...
void Model::prepare_regressive() {
  ...
  if (_axis_map["past_sequence_length"] == 0) {
    _axis_map["past_sequence_length"] = _axis_map["seq_len"];
    _axis_map["seq_len"] = 1;
    _axis_map["past_sequence_length + sequence_length"]++;
  } else {
    _axis_map["past_sequence_length"]++;
    _axis_map["seq_len"] = 1;
    _axis_map["past_sequence_length + sequence_length"]++;
  }
