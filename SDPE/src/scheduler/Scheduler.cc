      ...
      if (tile->status == Tile::Status::BAR) {
        ...
        if (stat.launched_tiles == stat.finished_tiles) {
          ...
          if (!_active_layers_map[layer_id].remain_tiles) {
            _active_layers_map[layer_id].remain_tiles++;
            finish_tile(core_id, layer_id, compute_cycle);
          } else {
...
void Scheduler::finish_tile(uint32_t core_id, int layer_id, int *compute_cycle) {
  ...
  std::regex EmbedLayerNormalization("^EmbedLayerNormalization.*");
  std::regex QKVGen("^QKV.*");
  std::regex GptAttention("^GptAttention_(?!.*matmul$).*");
  std::regex GptAttention_matmul(".*matmul$");
  std::regex SkipLayerNorm_AddBias("^SkipLayerNorm.*");
  std::regex FullyConnect_MatMul("^FullyConnect_MatMul_(\\d+).*");
  std::regex FastGelu_AddBias("^FastGelu.*");
  std::regex lm_head_MatMul("^/lm_head.*");
  if (_active_layers_map[layer_id].remain_tiles == 0) {
    _active_layers_map[layer_id].finish_cycle = *_core_cycle;
    trace_file.open("/workspace/ONNXim/traces/trace.txt",std::ios::app);
    if(std::regex_match(_active_layers_map[layer_id].name, QKVGen)){
        qkv_gen += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, EmbedLayerNormalization)){
        embedding += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, GptAttention)){
        attention += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, GptAttention_matmul)){
        projection += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, SkipLayerNorm_AddBias)){
        layernorm += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, FullyConnect_MatMul)){
      if(fc_count % 2 == 0){
        fullconnect1 += *_core_cycle - _active_layers_map[layer_id].start_cycle;
      }
      else{
        fullconnect2 += *_core_cycle - _active_layers_map[layer_id].start_cycle;
      }
      fc_count++;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, FastGelu_AddBias)){
        gelu += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    else if(std::regex_match(_active_layers_map[layer_id].name, lm_head_MatMul)){
        lm_head += *_core_cycle - _active_layers_map[layer_id].start_cycle;
    }
    //trace_file << "Layer "<< _active_layers_map[layer_id].name <<" finish at " << *_core_cycle <<std::endl;
    //trace_file << "Total compute time :" << *_core_cycle - _active_layers_map[layer_id].start_cycle << std::endl;
    //trace_file << "only compute time :" << *compute_cycle << std::endl;
    if(_active_layers_map[layer_id].name == "/lm_head/MatMul"){
      trace_file << "id : " << ++seq_length << "Layer " << _active_layers_map[layer_id].name << " finish at " << *_core_cycle << std::endl;
      if((seq_length == 33)||(seq_length == 65)){
          trace_file << "embedding cycle : " << embedding << std::endl;
            trace_file << "qkv_gen cycle : " << qkv_gen << std::endl;
            trace_file << "attention cycle : " << attention << std::endl;
            trace_file << "projection cycle : " << projection << std::endl;
            trace_file << "layernorm cycle : " << layernorm << std::endl;
            trace_file << "gelu cycle : " << gelu << std::endl;
            trace_file << "fc-1 cycle : " << fullconnect1 << std::endl;
            trace_file << "fc-2 cycle : " << fullconnect2 << std::endl;
            trace_file << "lm_head cycle : " << lm_head << std::endl;
      }
    }
...
void TimeMultiplexScheduler::finish_tile(uint32_t core_id, int layer_id, int *compute_cycle) {
  spdlog::debug("Layer {} Core {} Finish Tile at {} Remain tile {}", layer_id, core_id,
                *_core_cycle, _active_layers_map[layer_id].remain_tiles);
  assert(_active_layers_map.find(layer_id) != _active_layers_map.end());
  assert(_active_layers_map[layer_id].remain_tiles > 0);
...
void HalfSplitScheduler::finish_tile(uint32_t core_id, int layer_id, int *compute_cycle) {
  assert(_active_layers_map.find(layer_id) != _active_layers_map.end());
  assert(_active_layers_map[layer_id].remain_tiles > 0);
  _active_layers_map[layer_id].remain_tiles--;
  _active_layers_map[layer_id].finished_tiles++;
