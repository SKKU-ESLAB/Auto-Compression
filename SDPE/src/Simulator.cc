...
void Simulator::cycle() {
    ...
    if (_cycle_mask & CORE_MASK) {
      ...
      for (int core_id = 0; core_id < _n_cores; core_id++) {
        ...
        if (finished_tile->status == Tile::Status::FINISH) {
          _scheduler->finish_tile(core_id, finished_tile->layer_id, &_cores[core_id]->compute_cycle);
        }
  ...
  trace_file << "embedding cycle : " << _scheduler->embedding << std::endl;
  trace_file << "qkv_gen cycle : " << _scheduler->qkv_gen << std::endl;
  trace_file << "attention cycle : " << _scheduler->attention << std::endl;
  trace_file << "projection cycle : " << _scheduler->projection << std::endl;
  trace_file << "layernorm cycle : " << _scheduler->layernorm << std::endl;
  trace_file << "gelu cycle : " << _scheduler->gelu << std::endl;
  trace_file << "fc-1 cycle : " << _scheduler->fullconnect1 << std::endl;
  trace_file << "fc-2 cycle : " << _scheduler->fullconnect2 << std::endl;
  trace_file << "lm_head cycle : " << _scheduler->lm_head << std::endl;
  trace_file.close();
  //_scheduler->attention
