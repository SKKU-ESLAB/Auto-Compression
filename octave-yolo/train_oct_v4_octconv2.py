...
parser.add_argument("--name", type=str, default=None, help="model log name")
...
else : log_dir = os.path.join('logs', now)
...
# 모델 준비하기
model = model.oct_yolov4_octconv2.YOLOv4(args.image_size, num_classes, args.alpha).to(device)
...
for epoch in tqdm.tqdm(range(args.epoch), desc='Epoch'):
    ...
    # checkpoint file 저장
    save_dir = os.path.join('checkpoints', log_dir)
    os.makedirs(save_dir, exist_ok=True)
    dataset_name = os.path.split(args.data_config)[-1].split('.')[0]
    if (AP.mean()>=best_map):
        torch.save(model.state_dict(), os.path.join(save_dir, 'best.pth'.format(dataset_name, epoch)))
        best_map = AP.mean()
    if(epoch ==args.epoch-1):
        torch.save(model.state_dict(), os.path.join(save_dir, 'last.pth'.format(dataset_name, epoch)))
