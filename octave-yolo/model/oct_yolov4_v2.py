...
class OctPANet(nn.Module):
    def __init__(self, feature_channels, alpha=0.5):
        ...
        self.downstream_conv5 = nn.Sequential(
            Conv_BN_ACT0a(in_channels=feature_channels[2]*2, out_channels=feature_channels[2]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=0.0, alpha_out=alpha, padding=0),
            ...
            Conv_BN_ACT(in_channels=feature_channels[2], out_channels=feature_channels[2], kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=0.5, padding=0)
        )
        self.downstream_conv4 = nn.Sequential(
        ...
        self.downstream_conv3 = nn.Sequential(
            ...
            Conv_BN_ACT(in_channels=feature_channels[0]//2, out_channels=feature_channels[0], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[0], out_channels=feature_channels[0], kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=0.5, padding=0),
        )
