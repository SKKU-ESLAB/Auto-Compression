    ...
    def __initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # print("initing {}".format(m))
class Oct2(nn.Module):
    def __init__(self, feature_channels):
        super(Oct2, self).__init__()
        self.Octaveblock = nn.Sequential(
            octconv2_v2.Conv_BN_ACT2(in_channels=256, out_channels=128, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1),
                                        alpha_l_in=8, alpha_m_in=2, alpha_h_in=1, alpha_l_out=4, alpha_m_out=2, alpha_h_out=1 ,padding=0),
            octconv2_v2.Conv_BN_ACT2(in_channels=128, out_channels=256, kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1),
                                        alpha_l_in=4, alpha_m_in=2, alpha_h_in=1, alpha_l_out=4, alpha_m_out=2, alpha_h_out=1 ,padding=1),
            octconv2_v2.Conv_BN_ACT2(in_channels=256, out_channels=128, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1),
                                        alpha_l_in=4, alpha_m_in=2, alpha_h_in=1, alpha_l_out=4, alpha_m_out=2, alpha_h_out=1 ,padding=0),
        )
    ...
    def forward(self, features):
        ...
        o_h, o_m, o_l = self.Octaveblock((features[0], features[1], features[2]))
        return [o_h, o_m, o_l]
...
class OctPANet(nn.Module):
    def __init__(self, feature_channels, alpha=0.5):
        ...
        self.downstream_conv5 = nn.Sequential(
            Conv_BN_ACT(in_channels=feature_channels[2]*2, out_channels=feature_channels[2]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=0.0, alpha_out=alpha, padding=0),
            ...
            Conv_BN_ACT(in_channels=feature_channels[2], out_channels=feature_channels[2]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=0.0, padding=0)
        )
        self.downstream_conv4 = nn.Sequential(
            ...
            Conv_BN_ACT(in_channels=feature_channels[1]//2, out_channels=feature_channels[1], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[1], out_channels=feature_channels[1]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=0.0 ,padding=0),
        )
        self.downstream_conv3 = nn.Sequential(
            Conv_BN_ACT(in_channels=feature_channels[0], out_channels=feature_channels[0]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=0.5, alpha_out=alpha, padding=0),
            Conv_BN_ACT(in_channels=feature_channels[0]//2, out_channels=feature_channels[0], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[0], out_channels=feature_channels[0]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=0),
            Conv_BN_ACT(in_channels=feature_channels[0]//2, out_channels=feature_channels[0], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[0], out_channels=feature_channels[0]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=0.0, padding=0),
        )
        ...
        self.upstream_conv4 = nn.Sequential(
            ...
            Conv_BN_ACT(in_channels=feature_channels[1], out_channels=feature_channels[1]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=0),
            Conv_BN_ACT(in_channels=feature_channels[1]//2, out_channels=feature_channels[1], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[1], out_channels=feature_channels[1]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=1.0, padding=0),
        )
        self.upstream_conv5 = nn.Sequential(
            ...
            Conv_BN_ACT(in_channels=feature_channels[2]//2, out_channels=feature_channels[2], kernel_size=3, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=alpha, padding=1),
            Conv_BN_ACT(in_channels=feature_channels[2], out_channels=feature_channels[2]//2, kernel_size=1, bias=False, activation_layer=nn.LeakyReLU(negative_slope=0.1), alpha_in=alpha, alpha_out=1.0, padding=0),
        )
        self.__initialize_weights()
    ...
    def forward(self, features):
        ...
        downstream_feature5, _downstream_feature5 = self.downstream_conv5(features[2])
        downstream_feature4, _downstream_feature4 = self.downstream_conv4((features[1], self.resample5_4(downstream_feature5)))
        downstream_feature3, _downstream_feature3 = self.downstream_conv3((features[0], self.resample4_3(downstream_feature4)))

        _upstream_feature4, upstream_feature4 = self.upstream_conv4((self.resample3_4(downstream_feature3), downstream_feature4))
        _upstream_feature5, upstream_feature5 = self.upstream_conv5((self.resample4_5(upstream_feature4), downstream_feature5))
...
class YOLOv4(nn.Module):
    def __init__(self, image_size: int, num_classes: int, alpha = 0.5):
        ...
        self.panet = Oct2(feature_channels)
