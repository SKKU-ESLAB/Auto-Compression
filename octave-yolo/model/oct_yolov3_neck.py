...
class OCTYOLOv3_NECK(nn.Module):
    def __init__(self, image_size: int, num_classes: int):
        super(OCTYOLOv3_NECK, self).__init__()
        ...
        self.conv_block1 = self.make_octconv_block2(512, 128)
    ...
    def forward(self, x, targets=None):
        ...
        conv_block1, _conv_block1 = self.conv_block1((residual_output['residual_3_8'],conv_block2))
    ...
    def make_octconv_block1(self, in_channels: int, out_channels: int):
        double_channels = out_channels * 2
        modules = nn.Sequential(
            self.make_octconv(in_channels=in_channels, out_channels=out_channels, kernel_size=1, padding=0, alpha_in=0, alpha_out=0.5),
            self.make_octconv(in_channels=out_channels, out_channels=double_channels, kernel_size=3),
            self.make_octconv(in_channels=double_channels, out_channels=out_channels, kernel_size=1, padding=0),
            self.make_octconv(in_channels=out_channels, out_channels=double_channels, kernel_size=3),
            self.make_octconv(in_channels=double_channels, out_channels=out_channels, kernel_size=1, padding=0, alpha_in=0.5, alpha_out=0)
        )
        return modules
    ...
    def make_octconv_block2(self, in_channels: int, out_channels: int):
        double_channels = out_channels * 2
        modules = nn.Sequential(
            self.make_octconv(in_channels=in_channels, out_channels=out_channels, kernel_size=1, padding=0, alpha_in=0.5, alpha_out=0.5),
            self.make_octconv(in_channels=out_channels, out_channels=double_channels, kernel_size=3,alpha_in=0.5, alpha_out=0.5),
            self.make_octconv(in_channels=double_channels, out_channels=out_channels, kernel_size=1, padding=0,alpha_in=0.5, alpha_out=0.5),
            self.make_octconv(in_channels=out_channels, out_channels=double_channels, kernel_size=3,alpha_in=0.5, alpha_out=0.5),
            self.make_octconv(in_channels=double_channels, out_channels=out_channels, kernel_size=1, padding=0, alpha_in=0.5, alpha_out=0)
        )
        return modules
...
if __name__ == '__main__':
    model = OCTYOLOv3(image_size=416, num_classes=80)
