...
class Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,fft=False):
    ...
    def forward(self, x):
        ...
        if self.fft:
            with torch.no_grad():
                fft_result = torch.fft.fftn(x, dim=[2, 3])
                # FFT 결과의 매그니튜드 계산
                magnitude = torch.abs(fft_result)
                # 매그니튜드의 평균 계산
                avg_magnitude = torch.mean(magnitude)
                # 여기에서 필요한 추가 동작을 수행할 수 있음 (예: 로깅, 시각화 등)
                print(avg_magnitude.item())
        return x
