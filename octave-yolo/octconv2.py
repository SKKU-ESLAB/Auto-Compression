...
class OctaveConv2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, alpha_in=0.25, alpha_out=0.25, stride=1, padding=0, dilation=1,
                 groups=1, bias=False):
        ...
        self.conv_l2l = None if alpha_out == 0 else \
                        nn.Conv2d(2*math.ceil(alpha_in * in_channels), 2*(math.ceil(alpha_out * out_channels)),
                                  kernel_size, 1, padding, dilation, math.ceil(alpha_in * groups), bias)
        self.conv_l2m = None if alpha_out == 0 else \
                        nn.Conv2d(2*math.ceil(alpha_in * in_channels), (math.ceil(alpha_out * out_channels)),
                                  kernel_size, 1, padding, dilation, math.ceil(alpha_in * groups), bias)
        self.conv_l2h = None if alpha_out == 1 or self.is_dw else \
                        nn.Conv2d(2*math.ceil(alpha_in * in_channels), out_channels - 3*(math.ceil(alpha_out * out_channels)),
                                  kernel_size, 1, padding, dilation, groups, bias)
        
        self.conv_m2l = None if alpha_out == 0 or self.is_dw else \
                        nn.Conv2d(math.ceil(alpha_in * in_channels), 2*(math.ceil(alpha_out * out_channels)),
                                  kernel_size, 1, padding, dilation, groups, bias)
        ...
        self.conv_m2h = None if alpha_out == 1 else \
                        nn.Conv2d(math.ceil(alpha_in * in_channels), out_channels - 3*(math.ceil(alpha_out * out_channels)),
...
class Conv_BN2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, alpha_in=0.25, alpha_out=0.25, stride=1, padding=0, dilation=1,
        ...
        self.conv = OctaveConv2(in_channels, out_channels, kernel_size, alpha_in, alpha_out, stride, padding, dilation,
                               groups, bias)
        self.bn_h = None if alpha_out == 1 else norm_layer(out_channels - 3*(math.ceil(alpha_out * out_channels)))
        self.bn_m = None if alpha_out == 0 else norm_layer(math.ceil(out_channels * alpha_out))
        self.bn_l = None if alpha_out == 0 else norm_layer(2*math.ceil(out_channels * alpha_out))
