...
def window_partition_nine(x, window_size):
    B, C, H, W = x.shape
    x = x.view(B, C, H // window_size, window_size, W // window_size, window_size)
    windows = x.permute(0, 2, 4, 3, 5, 1).reshape(-1, window_size*window_size, C)
    return windows
...
def window_reverse(windows, window_size, H, W, B, shift = False):
    x = windows.view(B, H // window_size, W // window_size, window_size, window_size, -1)
    x = x.permute(0, 5, 1, 3, 2, 4).reshape(B, windows.shape[2], H, W)
    if shift:
        #[4:-3,4:-3]
        x = x[:, :, int(round(window_size / 2)) :-(window_size // 2), int(round(window_size / 2)):-(window_size // 2 )]
    return x
    ...
    def forward(self, input_tensor):
        ...
        if not self.grid_exists:
            ...
            if self.rank == 1:
                ...
                # relative_coords_h = [0, 1, 2, 3, 4, 5, 6]
                # relative_coords_w = [0, 1, 2, 3, 4, 5, 6]
                relative_coords_h = torch.arange(0, seq_length, device=input_tensor.device, dtype = input_tensor.dtype)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
           ...
           https://vpn.skku.edu:9000  attn_drop: attention dropout rate.
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None:
            BnW = mask.shape[0]
            attn = attn.reshape(B // BnW, BnW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.shift = shift
        if self.shift:
            mask = torch.ones((1, 1, (sr_ratio-1) * window_size, (sr_ratio-1) * window_size))
            mask = F.pad(mask, (int(round(self.window_size / 2)), self.window_size // 2, int(round(self.window_size / 2)), self.window_size // 2))
            mask = window_partition(mask, window_size)
            mask = torch.cat([torch.ones(sr_ratio * sr_ratio, cr_tokens_per_window, 1), mask], dim = -2)

            mask = mask @ mask.transpose(-1, -2)
            attn_mask = torch.zeros_like(mask)
            attn_mask = attn_mask.masked_fill(mask == 0., float(-10000.)).masked_fill(mask != 0., float(0.))
        else:
            attn_mask = None
        self.register_buffer("mask", attn_mask)
        
    def forward(self, x, carrier_tokens): 
        

        # B : batch * window // T : Height * Width (window) // N : # of channels
        B, T, N = x.shape
        ct = carrier_tokens
        x = self.pos_embed(x)
        ...
        x = x + self.drop_path(self.gamma3*self.attn(self.norm1(x), self.mask))
        ...
        if conv:
            ...
            self.blocks = nn.ModuleList([
                HAT(dim=dim,
                    ...
                    attn_drop=attn_drop,
                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                    sr_ratio=sr_ratio + 1 if (i % 2 == 1 and not only_local) else sr_ratio,
                    window_size=window_size,
                    last=(i == depth-1),
                    layer_scale=layer_scale,
                    ct_size=ct_size,
                    do_propagation=do_propagation,
                    shift = (i % 2 == 1 and not only_local),
                    )
                for i in range(depth)])
    ...
    def forward(self, x):
        for bn, blk in enumerate(self.blocks):
            ...
            if self.transformer_block:
                if bn % 2 == 1 and self.do_gt:
                    x = F.pad(x, (int(round(self.window_size / 2)), self.window_size // 2, int(round(self.window_size / 2)), self.window_size // 2))
            ...
            if self.transformer_block:
                x = window_reverse(x, self.window_size, H, W, B, (bn%2) == 1 and self.do_gt)
