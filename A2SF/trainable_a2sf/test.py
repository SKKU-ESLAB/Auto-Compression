...
class net(nn.Module):
  def __init__(self):
    ...
    self.topk = TopK_custom(20)
  
  def forward(self, x):
    x = self.fc(x).view(1, -1)
    topk_vector, _ = self.topk(x)

    return topk_vector.sum(dim=-1)

num_classes = 100
num_active_classes = 20

A = [torch.randn(100, 3).to(torch.float).to("cuda") for _ in range(num_classes)]
B = []

for _ in range(num_classes):
  vector = torch.zeros(num_classes)
  indices = torch.randperm(num_classes)[:num_active_classes]
  vector[indices] = 1

  B.append(vector.to("cuda"))
...
with tqdm(range(1000)) as pbar:
  for _ in pbar:
    ...
    for i in range(num_classes):
      loss = loss + criterion(mynet(A[i]), B[i])
