...
from vit_l2_3keep_senet_mlerp import VisionTransformerDiffPruning as VisionTransformerDiffPruningMLERP
...
def get_args_parser():
    ...
    parser.add_argument('--model_path', type=str, default="./deit_small_patch16_224-cd65a155.pth")
...
def main(args):
    ...
    elif args.arch == 'deit_small_mlerp':
        PRUNING_LOC = [3,6,9] 
        print(f"Creating model: {args.arch}")
        print('token_ratio =', KEEP_RATE, 'at layer', PRUNING_LOC)
        model = VisionTransformerDiffPruningMLERP(
            patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, 
            pruning_loc=PRUNING_LOC, token_ratio=KEEP_RATE, distill=args.distill
            )
        model_path = args.model_path
