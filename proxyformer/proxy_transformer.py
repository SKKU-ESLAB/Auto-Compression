...
class attention_dropout(nn.Module):
    ...
    def forward(self, x):
        ...
        if (not self.training) or self.p == 0.:
            ...
            B, H, N1, N2 = x.shape
            mask = torch.full((B, H, N2), fill_value = self.p, dtype = x.dtype, device = x.device)
            mask = torch.bernoulli(mask).eq(1)
            masks = torch.zeros_like(x, device = x.device)
            masks.masked_fill_(mask.reshape(B, H, 1, N2), value = 1.)
            x = x - masks.float() * 1e3
        return x
...
class injection_module(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.dim = embed_dim

        self.qnorm = nn.LayerNorm(self.dim)
        self.knorm = nn.LayerNorm(self.dim)
        self.scale = self.dim ** -0.25

    def forward(self, Q, K, V):
        Q = self.qnorm(Q) * self.scale
        K = self.knorm(K) * self.scale
...
class ProxyAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., num_learned_tokens = 64):
        super().__init__()
        ...
        self.landmarks = nn.Linear(dim, dim, bias=qkv_bias)
        self.attn_drop = attention_dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.num_learned_tokens = num_learned_tokens
    def forward(self, x):
        B, tN, C = x.shape
        p, x = x[:, :self.num_learned_tokens, :], x[:, self.num_learned_tokens:, :]
        
        _, xN, _ = x.shape
        _, pN, _ = p.shape 

        l = self.landmarks(p).reshape(B, pN,self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        
        qkv = self.qkv(x).reshape(B, xN, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        #lqkv = self.qkv(p).reshape(B, pN, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)
        #lq, lk, _ = lqkv.unbind(0)

        k1 = (q @ l.transpose(-2, -1)) * self.scale
        k2 = (l @ k.transpose(-2, -1)) * self.scale
        k2 = self.attn_drop(k2)
        ...
        x = torch.cat((po, xo), dim = -2).transpose(1, 2).reshape(B, tN, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, (l, )
...
class Block(nn.Module):
    ...
    def forward(self, x):
        mha, l = self.attn(self.norm1(x))
        x = x + self.drop_path1(self.ls1(mha))
...
class Proxyformer(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        assert global_pool in ('', 'avg', 'token', 'proxy')
        ...
        self.num_learned_tokens = 64
        self.injection_module = injection_module(embed_dim)
        self.proxy_token = nn.Parameter(torch.randn(1, self.num_learned_tokens, embed_dim) * .02)
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_token', 'dist_token', 'proxy_token'}
    ...
    def _pos_embed(self, x):
        if self.no_embed_class:
            ...
            if self.cls_token is not None:
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
            x = x + self.pos_embed
        p = self.proxy_token + self.injection_module(self.proxy_token, x, x)
        x = torch.cat((p, x), dim = 1)
        #x = torch.cat((self.proxy_token.expand(x.shape[0], -1, -1), x), dim = 1)
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if self.global_pool:
            if self.global_pool == "avg":
                ...
                x = x[:,self.num_learned_tokens]
            elif self.global_pool == "proxy":
                x = x[:, :self.num_learned_tokens].mean(dim = 1)
        x = self.fc_norm(x)
        return x if pre_logits else self.head(x)

    def forward(self, x):
        x,ls = self.forward_features(x)
...
@torch.no_grad()
def _load_weights(model: Proxyformer, checkpoint_path: str, prefix: str = ''):
