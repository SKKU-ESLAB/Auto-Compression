...
bool ExecuteKernel(uint8_t *pim_x, uint8_t *pim_y, uint8_t *pim_z, PIM_CMD pim_cmd)
{
	switch (pim_cmd)
	{
	...
	case (PIM_CMD::READ_WEIGHT_8COL):
		...
		for (int ch = 0; ch < NUM_CHANNEL; ch++)
		{
			uint64_t hex_addr = GetAddress(ch, 0, 0, 0, 0, 0);
				...
				thr_grp_param[ch].pim_addr = pim_y + hex_addr;  // 512 : 2 << co_pos << shift_bits
			...
			if (FpgaMode())
			{
				for (int i = 0; i < 8; i++)
				{
					uint64_t tmp = hex_addr + 32 * i * 256;
					WriteReg(PIM_REG::ADDR, (uint8_t *)&tmp, false);
				}
				for (int i = 0; i < 8; i++)
				{
			...
			else if (ComputeMode())
			{
				for (int i = 0; i < 8; i++)
				{
					pim_func_sim->AddTransaction((uint64_t)(pim_y + hex_addr + 32 * i * 256), data_temp_, false);
				}
			}
			else
			{
...
void TryAddTransaction(uint8_t *pim_addr, uint8_t *data, bool is_write)
{
	if (is_write)
	{
		if (FpgaMode())
			...
			pim_func_sim->AddTransaction((uint64_t)(pim_addr - pim_mem), data, is_write);
		else
			std::memcpy(pim_addr, data, burstSize_);
	}
	else
