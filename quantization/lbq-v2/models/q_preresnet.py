...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inp, outp, stride, input_size):
        ...
        if not self.residual_connection:
            self.shortcut = QuantizableConv2d(inp, outp, 1, stride=stride, bias=False, input_size=input_size)
...
class Model(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        #self.block_type = getattr(FLAGS, 'block', 'basic')
        ...
        for idx, n in enumerate(self.block_setting):
            outp = int(feats[idx]*block.expansion)
            for i in range(n):
                if i == 0 and idx != 0:
