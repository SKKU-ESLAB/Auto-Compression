...
def topk_conv2d_v2(N, I, H, W, O, dtype="float32"):
    ...
    conv = te.compute((N, K, H, W),
                     ...
                     name="conv")
    return [data, kernel, c_list, conv]
...
def topk_conv2d(N, I, H, W, O, dtype="float32"):
    ...
    cfg.define_knob("tile_co", [1, 2, 4, 8, 16])
    ...
    conv = te.compute(
        ...
        #lambda n, co, h, w, vh, vw: te.sum(
            ...
            * kernel_vec[c_list[co*VC+vc], ci, kh, kw].astype("float32"),
            axis=[ci, kh, kw],
        ),
        name="conv",
    )
...
task_inputs = [tvm.nd.array(data), tvm.nd.array(weight), tvm.nd.array(c_list[:64]), tvm.nd.array(out)]
task = tvm.auto_scheduler.SearchTask(func=topk_conv2d_v2, args=(N, I, H, W, O, "float32"), target=target, task_inputs=task_inputs, task_inputs_overwrite=True)
